...
const LetterDetail = () => {
  ...
  const textDummy = [
    '사랑하는 너에게, 오늘 아침 눈을 뜨자마자 달력을 보니 우리 기념일이라는 사실에 마음이 설렌다. 우리가 함께 보낸 시간이 벌써 이렇게 쌓였다는 게 믿기지 않을 정도로 빠르게 느껴져. 처음 만났던 순간부터 지금까지 함께한 모든 기억들이 하나둘 떠오르면서 마음이 참 따뜻해졌어.',
    '특히 처음으로 둘이 떠났던 그 여행 기억나? 분명히 잘 계획한 줄 알았는데 지도 앱 때문에 엉뚱한 길로 빠져서 두 시간이나 헤맸잖아. 그때는 조금 당황스럽기도 했지만, 오히려 그 덕분에 우리는 더 많은 얘기를 나눌 수 있었던 것 같아. 너와 함께라서 그런 작은 실수조차도 웃음으로 바뀌는 마법 같은 일이 벌어졌지.',
    '생각해보면 너와의 일상은 특별한 일들로만 채워지지 않아도 충분히 즐거워. 함께 본 영화가 지루해서 서로 장난치며 보냈던 시간도, 가끔 요리를 실패해 배달 음식을 주문하며 웃었던 날들도 모두 우리에겐 특별한 순간들이었어. 네가 곁에 있기에 평범한 일상조차도 소중한 추억이 된다는 걸 알게 되었어.',
    ...
    '언제나 내 곁에 있어줘서 정말 고맙고, 무엇보다 내가 너를 이렇게 많이 좋아하게 해줘서 정말 고마워. 우리의 기념일을 진심으로 축하해. 앞으로도 계속 너를 아끼고 사랑할 나로부터.',
    '2025년 3월 16일에 보낸 편지를',
    '2025년 3월 18일에 열었습니다.',
  ];
  ...
  const contents = contentConfig.map(({ template, imageCount, textCount, background }) => {
    ...
    const text = textDummy.slice(textIndex, textIndex + textCount);
